...
export default function AdminData() {
  ...
  const runExport = async (kind: "json" | "csv") => {
    ...
    try {
      ...
      const { file, results: resResults } = await res.json();
  ...
  const runImport = async () => {
    ...
    setError(null);
    setMode("import");
    setBusy("uploading data");
    try {
      const filteredTables: Record<string, any[]> = {};
      for (const [name, rows] of Object.entries(pending.file.tables)) {
        if (selected.includes(name)) {
          filteredTables[name] = rows;
        }
      }
