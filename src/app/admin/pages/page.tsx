...
export default function AdminPagesList() {
  ...
  const addSubmenu = async (parent: MenuNode, label: string, labelBn: string, href: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(d.error || "Failed to add submenu");
      }
      await refreshMenus();
    } catch (e: any) {
      setErr(e.message);
  ...
  const deleteMenuItem = async (item: MenuNode) => {
    ...
    await fetch(`/api/admin/menus?id=${item.id}`, { method: "DELETE" });
    await refreshMenus();
  };
  ...
  const reorderMenuItems = async (ids: string[]) => {
    ...
    await fetch("/api/admin/menus", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        items: ids.map((id, i) => ({ id, sort_order: i + 1 })),
      }),
    });
    await refreshMenus();
  };
    ...
    try {
      ...
      if (menuLoc !== "none") {
        ...
        if (!menuRes.ok) {
          ...
          setErr(`Page created, but adding it to the menu failed: ${md.error}`);
          await invalidate();
          await loadMenu();
          return;
        }
  ...
  const bulk = async (action: "publish" | "hide" | "delete") => {
    ...
    if (action === "delete") {
      ...
      await fetch(`/api/admin/pages?ids=${deletable.join(",")}`, { method: "DELETE" });
    } else {
      await fetch("/api/admin/pages", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ids: selected, is_published: action === "publish" }),
      });
    }
    setSelected([]);
    await invalidate();
                    ...
                    <tr className="border-b border-border/50 hover:bg-secondary/30">
                      ...
                      <td className="px-3 py-3">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <Link
                            href={`/admin/pages/${p.page}?tab=navigation`}
