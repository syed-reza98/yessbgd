...
export default function AdminSettings() {
  ...
  const load = async () => {
    try {
      ...
      setDrafts(Object.fromEntries(list.map((r: Row) => [r.id, JSON.stringify(r.value ?? {}, null, 2)])));
    } catch (e) {
  ...
  const saveAll = async () => {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        payloadItems.push({ id: row.id, value: parsed });
      }
  ...
  const addSetting = async () => {
    ...
    try {
      const res = await fetch("/api/admin/settings", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ key, label: key, group: "custom", value: { text: "" }, sort_order: rows.length + 1 }),
      });
