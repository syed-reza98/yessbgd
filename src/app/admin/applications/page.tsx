...
const STATUS_STYLES: Record<Status, string> = {
  ...
  hired: "border-purple-500/30 bg-purple-500/10 text-purple-400",
};
...
export default function AdminApplications() {
  ...
  const downloadResume = (path: string, name: string) => {
  ...
  const bulkDelete = async () => {
    ...
    setBulkBusy(true);
    try {
      const res = await fetch(`/api/admin/applications?ids=${selected.join(",")}`, { method: "DELETE" });
  ...
  const updateNote = async (app: Application, status_note: string) => {
    ...
    try {
      const res = await fetch("/api/admin/applications", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: app.id, status_note }),
      });
